...
from shared_utils.util import to_chunks
...
class UnirefToSqliteParser:
    ...
    def parse(self):
        ...
        if self.verbose:
            print('Ignored the following unrecognized GO annotations: %s' % self.unrecognized_go_annotations)
            print('Parsed %d records with any GO annotation.' % self.n_records_with_any_go_annotation)
...
def create_h5_dataset(protein_annotations_sqlite_db_file_path, fasta_file_path, go_annotations_meta_csv_file_path, output_h5_file_path, shuffle = True, \
    ...
    if verbose:
        print('Will encode the %d most common annotations.' % len(common_annotation_ids))
    ...
    if verbose:
        print('Will create an h5 dataset of %d final sequences.' % n_seqs)
...
def load_seqs_and_annotations(protein_annotations_sqlite_db_file_path, fasta_file_path, shuffle = True, records_limit = None, verbose = True, \
    ...
    if verbose:
        print('Loading %s records...' % ('all' if records_limit is None else records_limit))
    ...
    if verbose:
        print('Loaded %d proteins and their GO annotations (%d columns: %s)' % (raw_proteins_and_annotations.shape + (', '.join(raw_proteins_and_annotations.columns),)))
    
    if shuffle:
        raw_proteins_and_annotations = raw_proteins_and_annotations.sample(frac = 1, random_state = 0)

    if verbose:
        print('Loading Faidx (%s)...' % fasta_file_path)
    ...
    for i, (_, (uniprot_id, raw_go_annotation_indices)) in enumerate(raw_proteins_and_annotations.iterrows()):
        ...
        if verbose and i % log_progress_every == 0:
            print('%d/%d' % (i, len(raw_proteins_and_annotations)), end = '\r')
    ...
    if verbose:
        print('Finished. Failed finding the sequence for %d of %d records.' % (n_failed, len(raw_proteins_and_annotations)))
