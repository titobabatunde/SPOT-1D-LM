...
def finetune(model_generator, input_encoder, output_spec, train_seqs, train_raw_Y, valid_seqs = None, valid_raw_Y = None, seq_len = 512, batch_size = 32, \
    ...
    if begin_with_frozen_pretrained_layers:
        print('Training with frozen pretrained layers...')
    ...
    print('Training the entire fine-tuned model...')
    ...
    if n_final_epochs > 0:
        print('Training on final epochs of sequence length %d...' % final_seq_len)
...
def filter_dataset_by_len(dataset, seq_len = 512, seq_col_name = 'seq', dataset_name = 'Dataset', verbose = True):
    ...
    if verbose:
        print('%s: Filtered out %d of %d (%.1f%%) records of lengths exceeding %d.' % (dataset_name, n_removed_records, len(dataset), 100 * n_removed_records / len(dataset), \
