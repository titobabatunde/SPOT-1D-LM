...
def train(model, dataloader, criterion, optimizer):
    ...
    for i, batch in enumerate(dataloader):
        ...
        # print(f'outputs shape: {outputs.shape} and y shape: {y.shape}')
        ...
        maxval = torch.argmax(outputs, axis=1)
        # print(f'argmax shape is {maxval.shape}')
        # check to see what's going on with accuracy
        num_correct      = (maxval == y).sum().item()
        num_samples      = y.shape[1] * y.shape[0]
...
for epoch in range(config['epoch']):
    ...
    if val_acc > (best_valacc):
        save_model(model, optimizer, scheduler, ['valid_acc', val_acc], epoch, f'checkpoint{config["run"]}_{epoch}.pth')
