...
class ModelTrainer:
    ...
    def setup(self, dataset_handler, resume_from = None):
        ...
        print('Starting with episode with seq_len = %d.' % starting_episode.seq_len)
        ...
        if resumed_weights_file_path is not None:
            with open(resumed_weights_file_path, 'rb') as f:
                ...
                print('Loaded weights from %s.' % resumed_weights_file_path)
    ...
    def train_next_epoch(self, autosave = True):
        ...
        if changed_episode:
            print('Starting a new episode with seq_len = %d.' % episode.seq_len)
            self.model_generator.dummy_epoch = self.epoch_generator.create_dummpy_epoch()[:2]
        ...
        print('Epoch %d (current sample %d):' % (self.current_epoch_index, self.epoch_generator.current_sample_index))
...
class EpisodeDataManager:
    ...
    def encode_next_epoch(self, log_length_dist = True):
        ...
        if log_length_dist:
            print('Epoch sequence length distribution (for seq_len = %d): %s' % (self.seq_len, \
